package com.example.kafka_reactive_play_test.sec02;
...
public class KafkaProducer {
    ...
    public static void main(String[] args) {
        ...
        var flux = Flux.interval(Duration.ofMillis(100))
                .take(100)
                .map(i -> new ProducerRecord<>("orders", i.toString(), "order-"+i))
                .map(pr -> SenderRecord.create(pr, pr.key()));

        var sender = KafkaSender.create(senderOptions);
        sender.send(flux)
                .doOnNext(sr -> log.info("offset: {}",sr.recordMetadata().offset()))
